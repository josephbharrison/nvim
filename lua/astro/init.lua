...
C = require "astro.colors"

local highlights = {}

for _, module in ipairs { "base", "lsp" } do
  highlights = vim.tbl_deep_extend("force", highlights, require("astro." .. module))
end

for plugin, enabled in
  pairs(user_plugin_opts("astro.plugins", {
    aerial = true,
    beacon = false,
    bufferline = true,
    cmp = true,
...
do
  if enabled then highlights = vim.tbl_deep_extend("force", highlights, require("astro.plugins." .. plugin)) end
end

for group, spec in pairs(user_plugin_opts("astro.highlights", highlights)) do
